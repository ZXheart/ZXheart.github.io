...
去他的！我们将使用这个粗糙的定义（看起来和语言规范的定义相同，知识改变了措辞）：一个*类型*是一组固有的，内建的性质，对于引擎**和开发者**来
说，它独一无二地标识了一个特定的值的行为，并将它与其他值区分开。

换句话说，如果引擎和开发者看待值`42`（数字）与看待`"42"`（字符串）的方式不同，那么这两个值就用有不同的*类型*——分别是`number`和`string`。
...
*当前*还不拥有值的变量，实际上有用`undefined`值。对这样的变量调用`typeof`将会返回`"undefined"`：
...
还有一个`typeof`与未声明变量关联的特殊性为，进一步增强了这种困惑。考虑这段代码：
...
`typeof`操作符甚至为“undeclared”（或“not defined”）变量返回`"undefined"`。要注意的是，当我们执行`typeof b`时，即使`b`是一个为声
明变量，也不会有错误被抛出。这是`typeof`的一种特殊的安全防卫行为。

和上面类似地，要是`typeof`与为声明变量一起使用时返回“undeclared”就好了，而不是将其结果值与不同的“undefined”情况混为一谈。

### `typeof` undeclared

不管怎样，当在浏览器中处理 JS 时这种安全防卫是一种有用的特性，因为浏览器中多个脚本文件会将变量加载到共享的全局命名空间。

**注意：** 许多开发者相信，在全局命名空间绝不应该有任何变量，而且所有东西应当被包含在模块和私有/隔离的命名空间中。这在理论上很伟大但在实践
...
消息这样的调试任务之前，你想要检查这个变量是否被声明了。一个顶层的全局`var DEBUG = true`声明只包含一个“debug.js”文件中，这个文件仅在你
...
**注意：** 如果你在为一个还不存在的特性定义一个“填补”，你可能想要避免使用`var`来声明`atob`。如果你在`if`语句内部声明`var atob`，即使这
...
这里，`FeatureXYZ`根本不是一个全局变量，但我们仍然使用`typeof`的安全防卫来使检查变为安全。而且重要的是，我们在这里*没有*可以用于检查的
