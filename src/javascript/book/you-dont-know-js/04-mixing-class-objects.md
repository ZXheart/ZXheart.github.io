...
论与真实且实际的 JS 代码联系起来。但是这里首先要帑过许多概念和假想代码吗，所以可别跟丢了 —— 坚持下去！
...
另一个关于类的关键概念是`多态（polymorphism）`，它描述这样的想法：一个来自于父类的泛化行为可以被字类覆盖，从而使它更加具
...
（高级）设计模式的底层机制 l，好像对所有代码来说 OO 是一个给定的基础。
...
我们定义`Vehicle`类，假定他有一个引擎，有一个打开打火器的方法，和一个行驶的方法。但你永远也不会制造一个泛化的“载具”，所
以在这里她只是一个概念的抽象。
...
可以中`ignition()`中具体看出多态的一个有趣的含义。在`pilot()`内部，一个相对多态引用指向了（被继承的）`Vehicle`版本
...
从概念上讲，子类可以使用相对多态引用（也就是`super`）来访问它父类的行为。然而在现实中，子类不过是被给予了一份从它父类继
承来的行为的拷贝而已。如果子类“覆盖”一个它继承的方法，原版的方法和覆盖版的方法实际上都是存在的，所以它们都是可以访问的。
...
用`this.drive()`直接访问它。被选用的标识符重叠**遮蔽**就是为什么我们不得不使用更复杂的*显式假想多态（explicit
pseudo-polymorphism）*的原因。
