...
1. **分词/词法分析**：将一连串字符打断成（对于语言来说）有意义的片段，成为 token（记号）。举例来说，考虑这段程序：`var a = 2;`。这段程序
   很可能会被打断成如下 token：`var`，`a`，`=`，`2`，`;`。空格也许会被保留为一个 token，这要看它是否是有意义的。

   ::: tip

   分词和词法分析之间的区别是微妙和学术上的，其中心在于这些 token 是否以*无状态*或*有状态*的方式被识别。简而言之，如果分词器去调用有状态的解析规
   则来弄清`a`是否应当被考虑为一个不同的 token，还是只是其他 token 的一部分，那么这就是**词法分析**。
...
和大多数其它语言的编译器一样，JS 引擎要比这区区三步复杂太多了。例如，在解析和代码生成的处理中，一定会存在优化执行效率的步骤，包括压缩冗余元素，等等。
...
为了简单起见我们可以说，任何 JS 代码段在它执行之前（通常是*刚好*在它执行之前！）都必须被编译。所以，JS 编译器将把程序`var a = 2;`拿过来，并
...
3. \_作用域——：*引擎*的另一个朋友；收集并维护一张所有被声明的标识符（变量）的列表，并对当前执行中的代码如何访问这些变量强制实施一组严格的规则。
...
在我们的例子中，*引擎*将会对变量`a`实施一个“LHS”查询。另一种类型的查询成为“RHS”。
...
实际上，我们可以表述的更准确一点儿。对于我们的目的来说，一个 RHS 是难以察觉的，因为它简单地查询某个变量的值，而 LHS 查询是试着找到变量容器本身，
以便它可以赋值。从这种意义上说，RHS 的含义实质上不是*真正的*“一格赋值的右手边”，更准确地说，它只是意味着“不是左手边”。
...
就像一个代码块儿或函数被嵌套在另一个代码块儿或函数中一样，作用于被嵌套在其他的作用域中。所以，如果在直接作用域中找不到一个变量的话，*引擎*就
...
`b`的 RHS 引用不能再函数`foo`的内部被解析，但是可以在它的外围*作用域*（这个例子中是全局作用域）中解析。
...
> **_引擎_**：“嘿，`foo`外面的*作用域*，哦，你使全局*作用域*，好吧，酷。听说过`b`吗？我得到一个它的 RHS 引用。”
...
在 ES5 中被加入的“strict mode”，有许多与一般/宽松/懒慵模式不同的行为。其中之一就是不允许自动/隐含的全局变量创建。在这种情况下，将不会有全
局*作用域*的变量交回给 LHS 查询，并且类似于 RHS 的情况，*引擎*将抛出一个`ReferenceError`。
...
mode”[^note-strict-mode]），或者一个`ReferenceError`（如果是“strict mode”）。
