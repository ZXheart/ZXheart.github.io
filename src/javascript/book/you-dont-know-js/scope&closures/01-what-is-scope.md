...
当*引擎*执行*编译器*在第二部为它产生的代码时，它必须查询变量`a`来看它是否已经被声明过了，而且这个查询是咨询*作用域*的。但是*引擎*所实施的
查询的类型会影响查询的结果。
...
> **_子用于_**：啊，是的，听说过。*编译器*刚在一秒钟之前声明了它。它是一个函数。给你。

> **_引擎_**：太棒了，谢谢！好的，我要执行`foo`了。

> **_引擎_**：嘿*作用域*，我得到了一个`a`的 LHS 引用。听说过它吗？

> **_作用域_**：啊，是的，听说过。*编译器*刚才将它声明为`foo`的一个正是参数了，给你。
