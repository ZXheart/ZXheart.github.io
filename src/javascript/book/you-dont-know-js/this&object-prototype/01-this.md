...
然而，这种方法也类似地回避了对`this`的*真正*理解，而且完全依靠变量`foo`的此法作用域。
...
对`this`的含义第二常见的误解，是他不知怎的指向了函数的作用域。这是一个刁钻的问题，因为在某一种意义上它有正确的部分，而在
另外一种意义上，它是严重的误导。
...
何*被调用的，被传递了什么参数等信息。这个记录的属性值一，就是在函数执行期间将被使用的`this`引用。
