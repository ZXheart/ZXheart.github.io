...
使用`grid-auto-rows: 50px`来定义隐式网格轨道的高度为 50px。
...
2. 设置自动创建的行的大小。根据内容多少，自动创建的行高度最小为 50px，最大会根据内容多少自动调整
...
我们先前定义网格时，定义的是网格轨道，不是**网格线**。网格布局为我们创建带编号的网格线，精确地控制每一个网格元素。如下三
列两行网格中，包含**四纵**和**三横**网格线。
...
`row-start-1`和`row-end-3`表示从横 1 到横 3，即`common-item-1`占用 2 条横轨道的高度

```vue
<template>
  <div class="common-wrapper grid grid-cols-3 grid-auto-rows-50px">
    <div
      class="common-item <!-- [!code highlight:5] -->
    ...
    <div class="common-item">3</div>
    <div
      class="common-item <!-- [!code highlight:10] -->
...
`row-gap:10px; column-gap:20px;`等价于`gap:10px 20px;`表示行间距为 10px，列间距为 20px。
